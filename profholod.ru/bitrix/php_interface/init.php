...
function saleEmailSendAfterSetPropertyValues ($ELEMENT_ID, $IBLOCK_ID)
{
	...
	if(58 == $IBLOCK_ID){
		...
		$result = CIBlockElement::GetByID($ELEMENT_ID);

		if ($element = $result->GetNext()){

			AddMessage2Log("Element: NAME - {$element['NAME']} | ID - {$ELEMENT_ID}".PHP_EOL, "send_mail");

			if (129 === $element['IBLOCK_SECTION_ID']){
				$arSelect = Array("ID","IBLOCK_ID","NAME","DETAIL_TEXT");
				$arFilter = Array(
					"IBLOCK_ID"=>$IBLOCK_ID,
					"PROPERTY_SUBS_FILES"=>$ELEMENT_ID,
					"ACTIVE"=>"Y",
					"SECTION_ID"=>130, 
				...
				while($ob = $result->GetNextElement())
				{
					...
					$text = $text.$element["NAME"].' — обновлено '.date("d.m.Y").'</p>';
					...
					$text=$text.'<table border="0" cellpadding="0" cellspacing="0" width="100%" style="font-size: 12px;">
						...
						$db_props = CIBlockElement::GetProperty(58, $element["ID"], array("sort" => "asc"), Array("CODE"=>"FILE"));
						...
						$text = $text.'<tr valign="middle">
							...
							<td valign="middle" style="color: black; font-weight: bold; font-size: 12px; width: 40%; padding: 25px 10px; border-right: 1px solid #ff5000; border-bottom: 1px solid #ff5000">
								<p style="color: black; display: inline-block; font-weight: bold; font-size: 12px;">'.$element["NAME"].'</p>
							</td>
								<td style="width: 40%; padding: 25px 10px; border-bottom: 1px solid #ff5000;">'.$element["DETAIL_TEXT"].'
							</td>
						</tr>';
					...
					AddMessage2Log("Send mail. FILE - ".CFile::GetPath($ar_props["VALUE"]).PHP_EOL, "send_mail");
				}
			}
		}
	}

	runkit_constant_remove("LOG_FILENAME");
}
